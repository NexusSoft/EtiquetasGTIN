...
namespace Etiquetas_AGV
{
    ...
    {
        public static class Voice_Pick_Code
        {
            public static string Compute(string GTIN, string lot, DateTime? packDate)
            { 
                ushort crc = Crc16.ComputeChecksum(Encoding.ASCII.GetBytes(string.Format("{0}{1}{2}", GTIN, lot, packDate.HasValue ? packDate.Value.ToString("yyMMdd") : string.Empty))); 
